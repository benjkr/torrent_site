...
function buildModel(props: LibraryTorrentCardProps): CardModel {
...
function Cover({ model }: { model: CardModel }) {
  const { meta, imdbId } = model;
  return (
    <div className="relative w-[7.25rem] sm:w-32 shrink-0 self-stretch overflow-hidden bg-muted">
      {meta?.image ? (
        <img
          src={meta.image}
          alt=""
...
function StatsGrid({
  ...
  return (
    <div className="grid grid-cols-2 gap-x-3 gap-y-1 text-xs text-muted-foreground mb-2.5">
      ...
      <div className="truncate tabular-nums" title={formatBytes(t.size || 0)}>
