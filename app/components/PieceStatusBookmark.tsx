...
export default function PieceStatusBookmark({
  ...
  useEffect(() => {
    ...
    let cols = Math.max(1, Math.floor((maxW - 16) / (cell + gap)));
    let rows = Math.ceil(total / cols);
    while (rows * (cell + gap) + 16 > maxH && cell > 2) {
      cell -= 1;
      cols = Math.max(1, Math.floor((maxW - 16) / (cell + gap)));
  ...
  // Tiny 3×3 mosaic for the ribbon face
  const mosaic = displayPieces
  ...
  return (
    <HoverCard open={open} onOpenChange={(next) => setOpen(next)}>
      <HoverCardTrigger
        ...
        aria-label={
          pct != null ? `Pieces ${pct}% complete` : "Piece status"
        }
      >
      ...
      <HoverCardContent
        ...
        align="end"
        sideOffset={8}
        className="w-auto max-w-[min(100vw-2rem,340px)] p-0 overflow-hidden rounded-xl border-0 shadow-2xl ring-1 ring-black/8"
      >
